...
namespace nexus {
  ...
  SiPMHamamatsuVUV_Test::SiPMHamamatsuVUV_Test(): BaseGeometry(),
                                                  visibility_(1),
  ...
  {
    // Messenger
    msg_ = new G4GenericMessenger(this, "/Geometry/SiPMHamamatsuVUV_Test/",
                                  "Control commands of geometry SiPMHamamatsuVUV_Test.");

    G4GenericMessenger::Command& sipm_size_cmd =
      msg_->DeclareProperty("sipm_size", sipm_size_, "Size of the sensor");
    sipm_size_cmd.SetUnitCategory("Length");
    sipm_size_cmd.SetParameterName("sipm_size", false);
    sipm_size_cmd.SetRange("sipm_size>0.");

  }

  SiPMHamamatsuVUV_Test::~SiPMHamamatsuVUV_Test()
  {
  }

  void SiPMHamamatsuVUV_Test::Construct()
  {
    ...
    SetDimensions(G4ThreeVector(sipm_size_, sipm_size_, sipm_z));

    G4Box* sipm_solid = new G4Box("SiPMHmtsuVUV", sipm_size_/2., sipm_size_/2., sipm_z/2);
    ...
    // Active window
    G4double active_window_x = sipm_size_;
    G4double active_window_y = sipm_size_;
    ...
    G4double energies[entries] = {0.9 *eV, 6.2 *eV};
    ...
    if (!sdmgr->FindSensitiveDetector(sdname, false)) {
        ...
        if (sensor_depth_ == -1)
          G4Exception("[SiPMHamamatsuVUV_Test]", "Construct()", FatalException,
